...
11. `_targetHp`/`_targetMp`/`_targetSp`/`_targetHpDecimal`/`_targetMpDecimal`/`_targetSpDecimal`/`_targetBuffTurn`/`_targetRank`: `HP%`/`SP%`/`MP%`/`HP% in decimal`/`SP%  in decimal`/`MP%  in decimal`/`buffRemainTime`/`attackRank` of target monster
    ...
    3. `targetBossType`(see 12.) /`targetBuffTurn`/`targetHp`/`targetMp`/`targetSp`/`targetHpDecimal`/`targetMpDecimal`/`targetSpDecimal` can get maxim/minimum/'sign_sum(`sum(sign(value))`)' value from all alive monsters by using suffix `max/min/count` such as: `_targetBuffTurn_max_bleed`.
