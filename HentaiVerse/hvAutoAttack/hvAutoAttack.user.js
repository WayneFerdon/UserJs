  ...
  try {
    ...
    function optionBox() { // 配置界面
      ...
      optionBox.innerHTML = [
        ...
        '    <div><l0>时间</l0><l1>時間</l1><l2>If ETR</l2> ≤ <input class="hvAANumber" name="riddleAnswerTime" placeholder="3" type="text"><l0>秒，提交当前选中答案 或 为空时随机选中</l0><l1>秒，提交當前選中答案 或 為空時隨機選中</l1><l2>s submit chosen answers or random </l2> <input class="hvAANumber" name="riddleAnswerChoose" placeholder="0" type="text"><l0>个答案并提交(请注意提交错误的答案可能比不提交或只提交一个正确选项的惩罚更重)</l0><l1>个答案並提交(請注意提交錯誤的答案可能比不提交或只提交一個正確選項的懲罰更重)</l1><l2>answers if none is chosen.(Notice that submit wrong answers might causing more punishment than submit nothing or single correct one)</l2></div>',
        '  </div>',
