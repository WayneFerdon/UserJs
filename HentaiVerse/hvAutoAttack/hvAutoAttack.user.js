...
const standalone = ['option', 'arena', 'drop', 'stats', 'roundType', 'staminaLostLog', 'monsterStatus', 'battleCode', 'roundAll', 'roundAll', 'roundNow', 'disabled', 'stamina', 'staminaTime', 'supplyAlerted'];
...
function optionBox() { // 配置界面
  ...
  optionBox.innerHTML = [
    ...
    '  <div>1. <l0>初始权重=场上最大血量/目标血量×血量比例系数<br>血量比例系数</l0><l1>初始權重=場上最大血量/目標血量×血量比例係數<br>血量比例係數</l1><l2>BaseWeight = MaxHPOnField/TargetHP×HPRateFcator<br>HPRateFactor</l2><input class="hvAANumber" name="hpRateFactor" placeholder="10" type="text"></div>',
    '  <div>2. <l0>初始权重与下述各Buff权重相加</l0><l1>初始權重與下述各Buff權重相加</l1><l2>PW(X) = BaseWeight + Accumulated_Weight_of_Deprecating_Spells_In_Effect(X)</l2><br>',
    ...
    '  <div>3. <l0>优先选择权重最低的目标</l0><l1>優先選擇權重最低的目標</l1><l2>Choose target with lowest rank first</l2><br><l0>BOSS:Yggdrasil额外权重</l0><l1>BOSS:Yggdrasil額外權重</l1><l2>BOSS:Yggdrasil Extra Weight</l2><input class="hvAANumber" name="YggdrasilExtraWeight" placeholder="-100" type="text" style="width:40px"></div>',
...
function checkStamina(staminaLow = undefined) {
  ...
  if (stamina > staminaLow && ((stamina + 24 - (timeNow % 24)) > g('option').staminaLowWithReNat)) {
    return true;
  }
...
function CheckSupply() {
  ...
  const alerted = getValue('supplyAlerted');
  if (!checked && !alerted) {
    setValue('supplyAlerted', true);
    alert(`Needs supply:${needs}`);
  }
  if (checked && alerted) {
    delValue('supplyAlerted');
  }
  g('supplyChecked', checked);
}
...
function LoadArenas() {
  ...
  setTimeout(LoadArenas, _1d - (time(0) % _1d));
}

function checkBattleReady(method, staminaLow) {
  if (getValue('disabled') || [g('supplyChecked'), g('equipChecked')].includes(undefined)) {
...
function countMonsterHP() { // 统计敌人血量
  ...
  const hpMax = Math.max.apply(null, hpArray);
  const factor = g('option').hpRateFactor;
  ...
  for (i = 0; i < monsterStatus.length; i++) { // 死亡的排在最后（优先级最低）
    ...
    let weight = monsterStatus[i].hpNow / hpMax * factor; // 0~10，生命越低权重越低优先级越高
    if (yggdrasilExtraWeight && 'Yggdrasil' === gE('div.btm3>div>div', monsterBuff[i].parentNode).innerText) { // 默认设置下，任何情况都优先击杀群体大量回血的boss"Yggdrasil"
      weight += yggdrasilExtraWeight; // defalut -100
    }
    for (j in skillLib) {
